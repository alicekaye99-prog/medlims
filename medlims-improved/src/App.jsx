...
const DEFAULT_SIGS = {
  lab:[{role:"Pathologist",field:"pathologist",showLic:true},{role:"Med. Technologist",field:"medtech",showLic:true}],
};
...
async function downloadResultAsPDF(result, patient, hospitalInfo, silent=false, staff=[]) {
  ...
  if(tplSigs && tplSigs.length > 0) {
    ...
    const names=[result.pathologist||"",result.medtech||""];
    const lics=[result.pathologistLic?"Lic. No. "+result.pathologistLic:"",result.medtechLic?"Lic. No. "+result.medtechLic:""];
    const roles=["PATHOLOGIST","MED. TECHNOLOGIST"];
    positions.forEach((x,i)=>{
      doc.setDrawColor(...black);doc.setLineWidth(0.4);doc.line(x-28,SIG_Y,x+28,SIG_Y);
      const esigSrc=getEsig(names[i]);
      if(esigSrc&&names[i]){ try{doc.addImage(esigSrc,"AUTO",x-ESIG_W/2,SIG_Y-ESIG_H*0.6,ESIG_W,ESIG_H);}catch(e){} }
      const nl=names[i]+(lics[i]?",  "+lics[i]:"");
      doc.setFont("times","bold");doc.setFontSize(sigFS);doc.setTextColor(...navy);
      doc.text(nl,x,SIG_Y+3,{align:"center"});
      doc.setFont("times","normal");doc.setFontSize(sigFS-1);doc.setTextColor(...grey);
...
function AppMain({licData}){
  ...
  useEffect(()=>{
    setPatients(dbLoad("lims_p3",[]));
    setStaff(dbLoad("lims_s3",[]));
    setResults(dbLoad("lims_r3",[]));
  ...
  const sp=v=>{setPatients(v);dbSave("lims_p3",v);};
  const ss=v=>{setStaff(v);dbSave("lims_s3",v);};
  const sr=v=>{setResults(v);dbSave("lims_r3",v);};
        ...
        <div style={{flex:1,overflowY:"auto",padding:20,display:"flex",flexDirection:"column",gap:16}}>
          ...
          {curSection&&(
            <LabEntry key={curSection+(barcodeNav?.patientId||"")} section={curSection} secDef={secDef}
              ...
              prePatientId={barcodeNav?.section===curSection?barcodeNav.patientId:""}/>
          )}
...
/* ══════════════════════════════════════════════════════
   LAB ENTRY
══════════════════════════════════════════════════════ */
function LabEntry({section,secDef,tests,patients,staff,results,hospital,onSave,onPrint,preSelectedTests=null,prePatientId=""}){
  ...
  const [form,setForm]=useState(()=>{
    ...
    return {
      patientId:prePatientId||"",physician:"",pathologist:"",medtech:"",
  ...
  const clear=()=>{
    const newForm={patientId:"",physician:"",pathologist:"",medtech:"",datetime:nowDT(),ward:"",values:{},ticked:{},brands:{},countValues:{},remark:""};
      ...
      <Card>
        ...
        <div style={{padding:16,display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
            ...
            <Field label="Med. Technologist" style={{flex:1,minWidth:200}}>
...
function ReportsView({results,patients,staff,onPrint,onBatchPrint,onDelete,onEdit}){
  ...
  const startEdit=()=>{setEditLines(sel.lines.map(l=>({...l})));setEditMode(true);setEditWard(sel.ward||"");setEditPhysician(sel.physician||"");setEditMedtech(sel.medtech||"");setEditPathologist(sel.pathologist||"");setEditRemark(sel.remark||"");};
  const cancelEdit=()=>{setEditMode(false);setEditLines([]);setEditWard("");setEditPhysician("");setEditMedtech("");setEditPathologist("");setEditRemark("");};
  ...
  const filtered=getTabResults(activeTab);
              ...
              <>
                <Card>
                  ...
                  <div style={{padding:"12px 16px",display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"8px 16px",background:C.surface,borderBottom:`1px solid ${C.border}`}}>
                    ...
                    <div>
                      <div style={{fontSize:10,color:C.muted,fontWeight:600,textTransform:"uppercase",letterSpacing:".04em",marginBottom:2}}>Med. Tech.</div>
                      {editMode
